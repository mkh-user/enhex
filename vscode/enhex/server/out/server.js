...
connection.onDidChangeConfiguration(change => {
    if (hasConfigurationCapability) {
        ...
        globalSettings = ((change.settings.enhex || defaultSettings));
    }
...
const symbols = Object.entries(symbolSpecs).map(([label, spec]) => ({
    label: label,
    kind: spec.kind,
    data: label,
    detail: spec.detail,
...
const completionSymbols = symbols.map(({ label, kind, data }) => ({ label, kind, data }));
